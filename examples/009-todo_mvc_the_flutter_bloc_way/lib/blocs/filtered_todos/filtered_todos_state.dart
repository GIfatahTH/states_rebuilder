...
class FilteredTodosLoading extends FilteredTodosState {
  FilteredTodosLoading(ReactiveModel<TodosState> todosStateRM) {
    //Unsubscription is done automatically when the Injector injecting FilteredTodosState is disposed
    todosStateRM.listenToRM(
      (rm) {
        if (rm.hasData && rm.state is TodosLoaded) {
          RM.get<FilteredTodosState>().setState(
                (currentState) => FilteredTodosLoaded.updateTodos(
                  currentState,
...
class FilteredTodosLoaded extends FilteredTodosState {
  ...
  static Stream<FilteredTodosState> updateFilter(
    ...
    if (RM.get<TodosState>().state is TodosLoaded) {
      yield currentState.copyWith(
        filteredTodos: _mapTodosToFilteredTodos(
          (RM.get<TodosState>().state as TodosLoaded).todos,
          filter,
        ),
        activeFilter: filter,
      );
