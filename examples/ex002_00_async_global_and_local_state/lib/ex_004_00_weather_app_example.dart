...
class WeatherService {
  ...
  void fetchWeather(String cityName) async {
    _weatherRM.stateAsync = weatherRepository.state.fetchWeather(cityName);
  }

  void fetchDetailedWeather(String cityName) async {
    _weatherDetailedRM.stateAsync =
        weatherRepository.state.fetchDetailedWeather(cityName);
  }

  // Add the possibility to refresh the weather in detailed screen
  Future refreshDetailedWeather() async {
    return _weatherDetailedRM.setState(
      (s) async {
        // Recall the fetchWether
        fetchWeather(s!.cityName);
        // await for it
        await _weatherRM.stateAsync;
        // and recall the for the detailed weather
        return weatherRepository.state.fetchDetailedWeather(s.cityName);
      },
      // Ignoring waiting and error states
      stateInterceptor: (currentSnap, nextSnap) =>
          !nextSnap.hasData ? currentSnap : nextSnap,
    );
  }
}
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: RefreshIndicator(
        displacement: 10,
        onRefresh: () {
          return weatherService.refreshDetailedWeather();
        },
