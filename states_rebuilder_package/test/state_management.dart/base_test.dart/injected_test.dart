...
void main() {
  testWidgets('WHEN a sync value is injected '
      ...
      'AND check that onError is called', (tester) async {
  ...
  testWidgets('WHEN sync injected is mutated asynchronously (Future) '
  ...
  testWidgets('WHEN sync injected is mutated asynchronously (Stream) '
      'THEN it is in the waiting status while waiting for the future '
      'AND it has data if the future ends with data'
      'AND it has error if the future ends with error', (tester) async {
    final model = RM.inject<int>(() => 1);
  ...
  testWidgets('WHEN state is injected using RM.injectFuture '
  ...
  testWidgets('WHEN state is injected using RM.injectStream '
      'THEN it starts in the waiting status '
      'AND check that onWaiting is called'
  ...
  testWidgets('WHEN state is injected using RM.injectStream '
      ...
      'AND the state is refreshed', (tester) async {
    String? onWaitingValue;
    int? onDataValue;
    final model = RM.injectStream<int>(
      () => stream(3),
  ...
  group('Global Injected cross test and mock test', () {
    ...
    testWidgets('Third test', (tester) async {
      model1.injectMock(() => 2);
      expect(model2.state, 4);
      model2.state++;
      expect(model2.state, 5);
    });
    testWidgets('forth test', (tester) async {
      expect(model2.state, 2);
      model2.state++;
      expect(model2.state, 3);
    });

    testWidgets('First future', (tester) async {
      expect(futureModel.isWaiting, true);
      await tester.pump(Duration(seconds: 1));
      expect(futureModel.state, 1);
    });
    testWidgets('Second future', (tester) async {
      expect(futureModel.isWaiting, true);
      await tester.pump(Duration(seconds: 1));
      expect(futureModel.state, 1);
    });
    testWidgets('Third future', (tester) async {
      futureModel.injectFutureMock(() => future(10));
      expect(futureModel.isWaiting, true);
      await tester.pump(Duration(seconds: 1));
      expect(futureModel.state, 10);
    });
    testWidgets('Forth future', (tester) async {
      expect(futureModel.isWaiting, true);
      await tester.pump(Duration(seconds: 1));
      expect(futureModel.state, 1);
    });

    testWidgets('Fifth future', (tester) async {
    ...
    testWidgets('Third stream', (tester) async {
    ...
    testWidgets('Forth stream', (tester) async {
      expect(streamModel.isWaiting, true);
      await tester.pump(Duration(seconds: 1));
      expect(streamModel.state, 1);
      expect(streamModel.isDone, true);
    ...
    testWidgets('Fifth stream', (tester) async {
    ...
    testWidgets('Sixth stream', (tester) async {
  ...
  testWidgets('Test when skipWaiting', (tester) async {
    final model = RM.inject<int>(() => 0);
    final widget = Directionality(
      textDirection: TextDirection.ltr,
      child: OnReactive(() {
  ...
  testWidgets('Manual status flag setting for injected state', (tester) async {
    SnapState? currentState;
    SnapState? nextState;
    ...
    void fn() async {
      try {
        ...
        final data = await Future.delayed(const Duration(seconds: 1), () => 10);
        if (exception != null) {
          throw exception;
        }
  ...
  testWidgets('Manual status flag setting FOR REACTIVE MODEL', (tester) async {
    final model = 0.inj();
    final widget = Directionality(
      textDirection: TextDirection.ltr,
      child: OnReactive(() {
  ...
  testWidgets('WHEN notify is called'
      'THEN side effects are not called', (tester) async {
    int numberOfSideEffectCall = 0;
    int numberOfBuildCall = 0;
    final model = RM.inject<int>(
      () => 0,
      sideEffects: SideEffects(onSetState: (c) => numberOfSideEffectCall++),
    );
    // ignore: unused_result
    model.addObserver(listener: (c) => numberOfBuildCall++);
    expect(model.state, 0);
  ...
  testWidgets(
    ...
    (tester) async {
      final model = RM.injectStream(
        () => Future.delayed(const Duration(seconds: 1)).asStream(),
      );
      expect(model.isWaiting, true);
  ...
  testWidgets('WHEN error is thrown'
      'THEN the error getter get the error with the stack', (tester) async {
