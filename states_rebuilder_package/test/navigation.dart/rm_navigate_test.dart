...
void main() {
  ...
  testWidgets('Assertion not navigatorKey is not assigned', (tester) async {
    final widget = MaterialApp(routes: {'/': (c) => Text('Home')});
  ...
  testWidgets('navigate to remove until', (tester) async {
    ...
    await tester.pumpAndSettle();
    //
    RM.navigate.toAndRemoveUntil(Route1(''), untilRouteName: 'ROUTE2');
    await tester.pumpAndSettle();
  ...
  testWidgets('navigate to named remove  all', (tester) async {
    ...
    await tester.pumpAndSettle();
    //
    RM.navigate.toNamedAndRemoveUntil('Route3', arguments: 'data');
    await tester.pumpAndSettle();
    expect(find.text('Route3'), findsOneWidget);
    //
    RM.navigate.back();
  ...
  testWidgets('Nested route trail', (tester) async {
    final home = (c) => Text('/');
        ...
        routes: {
          ...
          '/page11': (c) {
            return RouteWidget(
              routes: {
                '/': (c) {
                  return page11(c.arguments);
                },
                '/page111': (c) {
                  return page111(c.arguments);
                },
  ...
  testWidgets('Test route is not found', (tester) async {
    final Map<String, Widget Function(RouteData)> routes = {
      '/page1': (c) => Text(''),
      '/page2': (c) => RouteWidget(builder: (c) => c),
      '/page3': (c) => RouteWidget(
        routes: {
          '/': (c) => Text(''),
          '/page31': (c) {
            return RouteWidget(builder: (c) => c);
          },
        },
      ),
      '/page4': (c) => RouteWidget(
