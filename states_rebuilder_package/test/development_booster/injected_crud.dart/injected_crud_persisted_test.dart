  ...
  persist: () => PersistState(
    ...
    fromJson: (json) {
      return (jsonDecode(json) as List)
          .map((mappedOrder) => Order.fromJson(mappedOrder))
          .toList();
    },
  ),
);
...
main() async {
  ...
  testWidgets('WHEN when no cached order and app starts'
      'THEN the app fetches for items from the repo '
      'Case OnReactive is used', (tester) async {
  ...
  testWidgets('WHEN there is one order cached'
  ...
  testWidgets('WHEN when no cached order and app starts'
      'THEN the app fetches for items from the repo '
      'Case OnCRUDBuilder is used', (tester) async {
  ...
  testWidgets('Test when testOrderCRUD is mocked with simple injected', (
    tester,
  ) async {
    isOnReactiveUsed = false;
                          ...
                          children: testOrderCRUD.state
                              .map(
                                (order) => ListTile(
                                  ...
                                  subtitle: Text(order?.orderName ?? 'NO NAME'),
                                ),
                              )
                              .toList(),
                        );
...
class Order {
  String id;
  String orderName;
  Order({required this.id, required this.orderName});
  ...
  factory Order.fromMap(Map<String, dynamic> map) {
    return Order(id: map['id'] ?? '', orderName: map['orderName'] ?? '');
  }
...
class OrderRepository implements ICRUD<Order, String> {
  ...
  Future<List<Order>> read(String? param) async {
    ...
    final fakeOrders = List.generate(
      5,
      (index) => Order(id: 'id-$index', orderName: 'Order No.#$index'),
