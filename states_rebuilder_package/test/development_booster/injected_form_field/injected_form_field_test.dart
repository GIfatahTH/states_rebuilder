...
void main() {
  testWidgets('Check when the field is focused and unfocused'
      'THEN the decorator get the right state', (tester) async {
  ...
  testWidgets('WHEN initial value of injectedForm is null'
  ...
  testWidgets('WHEN autovalidateMode is always'
      'THEN the field is validated on initialization', (tester) async {
  ...
  testWidgets('WHEN readOnly is true,'
    ...
    final field = RM.injectFormField(true, isReadOnly: true);
    final widget = MaterialApp(
      home: Scaffold(
  ...
  testWidgets('WHEN enabled is false,'
      'THEN the field is disabled', (tester) async {
    final field = RM.injectFormField(true, isEnabled: false);
    final widget = MaterialApp(
      home: Scaffold(
        body: OnFormFieldBuilder<bool>(
          listenTo: field,
  ...
  testWidgets('WHEN enabled is false,'
      'THEN the field is disabled. Using Form', (tester) async {
  ...
  testWidgets('WHEN enabled is false,'
      'THEN the field is not focusable', (tester) async {
    final field = RM.injectFormField(true, isEnabled: false);
    ...
    final widget = MaterialApp(
      home: Scaffold(
        body: Column(
          children: [
            TextField(autofocus: true, focusNode: focusNode1),
            OnFormFieldBuilder<bool>(
              listenTo: field,
              builder: (value, onChanged) {
                return CheckboxListTile(
  ...
  testWidgets('WHEN isEnabledRM of the form is defined'
      'THEN all form child  isEnable will be overridden', (tester) async {
    final form = RM.injectForm();
  ...
  testWidgets('enable and disable form', (tester) async {
    final form = RM.injectForm(isEnabled: false);
    final text = RM.injectTextEditing();
    final check = RM.injectFormField(true);
    final widget = MaterialApp(
  ...
  testWidgets('WHEN isReadOnlyRM of the form is defined'
      'THEN all form child  isEnable will be overridden', (tester) async {
    final form = RM.injectForm();
    final text = RM.injectTextEditing();
    final check = RM.injectFormField(true);
  ...
  testWidgets('Toggle from readOnly', (tester) async {
    final form = RM.injectForm(isReadOnly: true);
    final text = RM.injectTextEditing();
    final check = RM.injectFormField(true);
    final widget = MaterialApp(
    ...
    (tester) async {
      ...
      form = RM.injectForm(
        submissionSideEffects: SideEffects.onOrElse(
          ...
          orElse: (c) => form.isFormEnabled = true,
        ),
        submit: () => Future.delayed(1.seconds),
      );
      final widget = MaterialApp(
  ...
  testWidgets('readOnly input fields when form is submitting'
      'Case nested OnFormBuilder', (tester) async {
    final text = RM.injectTextEditing();
    final check = RM.injectFormField(true);
    ...
    final form = RM.injectForm(
      submissionSideEffects: SideEffects.onOrElse(
        ...
        orElse: (c) => isReadOnly.state = false,
      ),
      submit: () => Future.delayed(1.seconds),
    );
    final widget = MaterialApp(
  ...
  testWidgets('readOnly input fields when form is submitting'
      'Case nested OnFormBuilder using form.isFormReadOnly', (tester) async {
    final text = RM.injectTextEditing();
    final check = RM.injectFormField(true);
    late final InjectedForm form;
    form = RM.injectForm(
      submissionSideEffects: SideEffects.onOrElse(
        ...
        orElse: (c) => form.isFormReadOnly = false,
      ),
      submit: () => Future.delayed(1.seconds),
    );
    final widget = MaterialApp(
  ...
  testWidgets('issue 238', (tester) async {
    ...
    final widget = MaterialApp(
      ...
      onGenerateRoute: (routeSettings) {
        ...
        return MaterialPageRoute(
          builder: (c) => Text('other: ${isCheckedField.snapState.state}'),
        );
      },
    );
        ...
        return Column(
          children: [
            ...
            OnFormFieldBuilder<String>(
              listenTo: email,
              builder: (value, onChanged) {
                return TextFormField(initialValue: value, onChanged: onChanged);
              },
            ),
            OnFormFieldBuilder<bool>(
              listenTo: isValid,
