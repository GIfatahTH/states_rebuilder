...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      final widget = MaterialApp(
        home: Material(child: TextField(controller: textEditing.controller)),
      );
  ...
  testWidgets('Listen to the injected text editing', (tester) async {
    final widget = MaterialApp(
      home: Material(
        child: Column(
          children: [
            TextField(controller: textEditing.controller),
            OnBuilder(
  ...
  testWidgets('WHEN validator is define'
      'THEN input is validated', (tester) async {
      ...
      home: Material(
        child: OnBuilder(
          ...
          builder: () => TextField(
            ...
            decoration: InputDecoration(errorText: textEditing.error),
          ),
        ),
      ),
    );
  ...
  testWidgets('WHEN InjectedForm is used'
      ...
      'AND check form.isValid works', (tester) async {
    final widget = MaterialApp(
      home: Scaffold(
        body: OnFormBuilder(
          listenTo: form,
  ...
  testWidgets('WHEN InjectedForm is used'
      ...
      'AND check form.reset works', (tester) async {
    final form = RM.injectForm(autovalidateMode: AutovalidateMode.always);
  ...
  testWidgets('WHEN InjectedForm is used'
      ...
      'THEN the fields are validate on user interaction', (tester) async {
  ...
  testWidgets('WHEN autoDispose is set to false'
      'THEN the injected text editing is keeps its value', (tester) async {
    final name = RM.injectTextEditing(autoDispose: false);
  ...
  testWidgets('WHEN autoDispose is false'
      'THEN controller is preserved'
      'CASE TextField with no form nor listeners', (tester) async {
    final name = RM.injectTextEditing(autoDispose: false);
    final email = RM.injectTextEditing();
  ...
  testWidgets('focusNode works', (tester) async {
    final name = RM.injectTextEditing(autoDispose: false);
  ...
  testWidgets('WHEN Two form are defined with one has ListView builder'
      'THEN each form get the right associated TextFields', (tester) async {
    final form1 = RM.injectForm(autovalidateMode: AutovalidateMode.always);
    final form2 = RM.injectForm(autovalidateMode: AutovalidateMode.always);

    final textField1 = RM.injectTextEditing(
      validators: [(c) => 'TextField1 Error'],
    );
    final textField2 = RM.injectTextEditing(
      validators: [(c) => 'TextField2 Error'],
    );
    final widget = MaterialApp(
      home: Scaffold(
        body: Column(
          children: [
            ...
            OnFormBuilder(
              ...
              builder: () => TextField(
                ...
                decoration: InputDecoration(errorText: textField2.error),
              ),
            ),
          ],
        ),
  ...
  testWidgets('On.formSubmission widget and side effects work', (tester) async {
    final name = RM.injectTextEditing(validateOnLoseFocus: false);
    final email = RM.injectTextEditing(validateOnTyping: false);
    ...
    final form = RM.injectForm(
      ...
      submissionSideEffects: SideEffects.onOrElse(
        ...
        orElse: (c) => submitMessage = 'Submitted',
      ),
    );
  ...
  testWidgets(
    ...
    (tester) async {
      final name = RM.injectFormField<String>('', validateOnLoseFocus: false);
      final email = RM.injectFormField<String>('', validateOnLoseFocus: false);
      String submitMessage = '';
      String? serverError = 'Server Error';
      late void Function() refresher;
      final form = RM.injectForm(
  ...
  testWidgets('OnFormBuilder and OnFormSubmissionBuilder ', (tester) async {
    final name = RM.injectTextEditing(validateOnLoseFocus: false);
    final email = RM.injectTextEditing(validateOnTyping: false);
    String submitMessage = '';
    String? serverError = 'Server Error';
  ...
  testWidgets('WHEN  TextField is removed'
      'THEN it will be removed from form text fields list', (tester) async {
    final form = RM.injectForm();
      ...
      home: Scaffold(
        body: OnFormBuilder(
          ...
          builder: () {
            return Column(
              children: [
                TextField(controller: password.controller),
                if (isRegister.state)
                  TextField(controller: confirmPassword.controller),
              ],
            );
          },
        ),
  ...
  testWidgets('Test controllerWithInitialText', (tester) async {
    final form = RM.injectForm();
  ...
  testWidgets('WHEN a field is autoFocused'
      'THEN the it is assigned to the form autoFocusedNode'
      'AND it is auto validated when lost focus', (tester) async {
              ...
              children: [
                OnReactive(
                  () => TextField(
                    ...
                    autofocus: true,
                    decoration: InputDecoration(errorText: text.error),
                  ),
                ),
                ElevatedButton(
  ...
  testWidgets('Check TextField validation and reset without form', (
    tester,
  ) async {
    final text1 = RM.injectTextEditing(
      text: 'initial text1',
          ...
          () => Column(
            children: [
              ...
              TextField(
                ...
                decoration: InputDecoration(errorText: text2.error),
              ),
            ],
          ),
  ...
  testWidgets('WHEN focus changes'
      'THEN InjectedForm emits notification'
      'Using OnFormBuilder'
      'issue #226', (tester) async {
    ...
    final redWidget = find.byWidgetPredicate(
      (widget) =>
          widget is InputDecorator && widget.decoration.fillColor == Colors.red,
    );
  ...
  testWidgets('WHEN readOnly is true,'
      'THEN the TextField is selectable, clickable but not editable', (
    tester,
  ) async {
    final field = RM.injectTextEditing(text: '0', isReadOnly: true);
    final widget = MaterialApp(
      home: Scaffold(
        body: TextField(
          controller: field.controller,
  ...
  testWidgets(
    ...
    (tester) async {
      final form = RM.injectForm(isReadOnly: true);
      final field = RM.injectTextEditing(text: '0');
  ...
  testWidgets('WHEN isEnable is true,'
      'THEN the TextField is selectable, clickable but not editable', (
    tester,
  ) async {
    final field = RM.injectTextEditing(text: '0', isEnabled: false);
    final widget = MaterialApp(
      home: Scaffold(
        body: OnReactive(() {
          return TextField(
  ...
  testWidgets(
    ...
    'THEN the TextField is selectable, clickable but not editable. Using Form',
    (tester) async {
      final form = RM.injectForm(isEnabled: false);
      final field = RM.injectTextEditing(text: '0');
  ...
  testWidgets('issue 256', (tester) async {
    final form = RM.injectForm(isEnabled: false);
    final field = RM.injectTextEditing(
      text: '0',
      // isEnabled: false,
  ...
  testWidgets('# issue 241', (tester) async {
        ...
        home: OnReactive(
          () => Scaffold(
            ...
            body: Center(
              child: Column(
                children: [
                  TextField(controller: pwdInj.controller),
                  ElevatedButton(
  ...
  testWidgets('Test is dirty', (tester) async {
    final form = RM.injectForm();
    final email = RM.injectTextEditing();
    final password = RM.injectTextEditing();
      ...
      builder: () {
        return Column(
          children: [
            ...
            TextField(controller: email.controller),
            TextField(controller: password.controller),
          ],
        );
      },
