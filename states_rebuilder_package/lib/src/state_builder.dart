...
class _StateBuilderState<T> extends State<StateBuilder<T>>
  ...
  ReactiveModel<T> get _exposedModel => T == dynamic
...
void _initState<T>(_StateBuilderState<T> state) {
  ...
  void _resolveModels<T>(_StateBuilderState<T> state,
    ...
    state.widget.activeRM.addAll(state._models.toList());
...
void _dispose<T>(_StateBuilderState<T> state) {
  final widget = state.widget;
