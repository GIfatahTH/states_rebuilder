...
class _StateBuilderState<T> extends State<StateBuilder<T>>
  ...
  void initState() {
    ...
    if (widget.observeMany != null) {
      _models ??= {};
      _models.addAll(widget.observeMany.map((o) => o?.call()));
    }
    ...
    if (_models != null) {
      ...
      _exposedModelFromGenericType ??=
          _models.isNotEmpty && _models.first is ReactiveModel
              ? _models.first
              : null;
  ...
  _subscribe(StatesRebuilder model) {
    assert(model != null);
    assert(() {
      if (model is RMKey && model.rm == null) {
        throw Exception('Non initialized RMKey\n'
            'Make sure to associate the RMKey with an observer widget');
      }
      return true;
    }());
    ...
    if (T != dynamic) {
      _exposedModelFromGenericType ??= model is ReactiveModel<T> ? model : null;
    }
  }
